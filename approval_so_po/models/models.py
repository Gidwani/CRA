...
class ResPartnerInh(models.Model):
    ...
    def write(self, vals):
        ...
        if not self.env.su and 'fromso' not in self._context and 'Iscreated' not in self._context and self.env.user.has_group('approval_so_po.group_contact_user') and 'copy' not in pre_name and self.active:
