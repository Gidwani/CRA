odoo.define('odoo_advance_search.utils', function (require) {
    ...
    function setAsRecordSelect($select, data=false) {
        ...
        $select.attr("value", window.advance_search[$select.attr("name")])
        $select.data('new_id_vals', window.advance_search[$select.attr('name')])
        ...
        $select.off('change').on('change', function(e) {
            ...
            $(document.getElementsByClassName('many2one_field')).click()
        });
